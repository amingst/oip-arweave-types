...
function splitTypeScriptIntoFiles(content: string): { files: Record<string, string>; indexContent: string } {
	...
	for (const line of lines) {
		const exportMatch = line.match(/^export (?:type|interface) ([A-Za-z0-9_]+)/);
		if (exportMatch) {
	...
	for (let i = 3; i < lines.length; i++) {
		...
		if (exportMatch) {
			...
			if (currentName && currentBlock.length > 0) {
				...
				const imports = findRequiredImports(blockContent, allExports, currentName);
				const fileContent = header + (imports.length > 0 ? imports.join('\n') + '\n\n' : '') + blockContent + '\n';
	...
	const indexContent = header + exports.map(name => `export * from './${name}';`).join('\n') + '\n';
	
	return { files, indexContent };
}

// Helper function to find required imports for a block
function findRequiredImports(blockContent: string, allExports: string[], currentName: string): string[] {
	...
	for (const exportName of allExports) {
		if (exportName !== currentName) {
			...
			if (regex.test(blockContent)) {
				imports.push(`import type { ${exportName} } from './${exportName}';`);
			}
		}
	}
...
export async function fetchOipTemplates(
	...
	try {
		...
		if (outputPath) {
			if (singleFile) {
				...
				if (!path.extname(finalOutputPath)) {
					finalOutputPath = path.join(finalOutputPath, 'generated-types.ts');
				}
		...
		if (singleFile) {
			...
			const { files, indexContent } = splitTypeScriptIntoFiles(typeScriptContent);
			...
			console.log(chalk.yellow(`🔧 Generated ${Object.keys(files).length} separate files + index.ts`));
			console.log(chalk.yellow(`🔧 Retrieved ${interfaceCount} interfaces`));

			// Show directory stats
			const totalSize = Object.values(files).reduce((sum, content) => sum + (content as string).length, 0) + indexContent.length;
